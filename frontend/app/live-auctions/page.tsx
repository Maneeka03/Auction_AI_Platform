...
import { listAuctions, listPublicAuctions } from "@/lib/api/auctions";
...
function AuctionCard({ auction, href }: { auction: Auction; href: string }) {
  return (
    <Link
      href={href}
      ...
      {isLoading ? (
        ...
        <div className="grid grid-cols-1 gap-4 sm:grid-cols-2 lg:grid-cols-3">
          {auctions.map((auction) => (
            <AuctionCard key={auction.id} auction={auction} href={hrefFor(auction)} />
          ))}
        </div>
      )}
    </div>
...
export default function LiveAuctionsPage() {
  const { session, accessToken, isLoading: authLoading } = useAuth();
  ...
  const fetchAuctions = useCallback(async () => {
    ...
    try {
      const result = session
        ? await listAuctions(accessToken!, { page: 1, size: 50, status: activeTab === "all" ? undefined : activeTab })
        : await listPublicAuctions({ page: 1, size: 12, status: activeTab === "all" ? undefined : activeTab });
  ...
  }, [session, accessToken, authLoading, activeTab]);
    ...
    return (
      <BuyerShell>
        ...
        <AuctionsGrid
          ...
          error={error}
          auctions={auctions}
          hrefFor={(a) => `/live-auctions/${a.id}`}
        />
      </BuyerShell>
    );
  }

  // Logged out: public marketing chrome, links to the public property page
  // (the gated bidding room isn't reachable without an account).
  return (
    <div className="min-h-screen bg-neutral-50">
      ...
      <div className="pt-24">
        <AuctionsGrid
          ...
          error={error}
          auctions={auctions}
          hrefFor={(a) => `/browse-properties/${a.property_id}`}
        />
      </div>
      <Footer />
    </div>

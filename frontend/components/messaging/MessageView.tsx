...
export function MessageView({ title, messages, currentUserId, readOnly, onSend, loading }: Props) {
  ...
  return (
    <div className="flex h-full flex-col">
      ...
      <div className="flex-1 overflow-y-auto px-5 py-4 space-y-1">
        {loading ? (
          <p className="text-center text-sm text-neutral-400 py-10">Loading messages…</p>
        ) : messages.length === 0 ? (
          <p className="text-center text-sm text-neutral-400 py-10">No messages yet.</p>
        ) : (
          messages.map((msg) => {
            const senderId = msg.sender_id;
            const isMine = senderId === currentUserId;
            const senderName = isGroupMessage(msg) ? msg.sender_name : msg.sender_name;

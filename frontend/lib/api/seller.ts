...
export function getMyBankDetails(token: string): Promise<BankAccount | undefined> {
  return apiClient.get("/api/v1/seller/bank-details", { accessToken: token });
}

export function saveBankDetails(token: string, payload: UpsertBankAccountRequest): Promise<BankAccount> {
  return apiClient.put("/api/v1/seller/bank-details", payload, { accessToken: token });
}

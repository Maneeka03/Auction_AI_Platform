...
- **Active branch:** `feature/push-notifications` (off latest `main`; pushed to origin)
...
- `3da34f8dcb3f` — teammate autogenerated migration (adjusts users/refresh_tokens constraints).
- `0011_push_subscriptions` — adds the `push_subscriptions` table and the `new_message`
  notification kind, for web (device) push notifications.
- `0012_auction_comments` — adds the `auction_comments` table for the live auction-room chat.
...
**Auction room live chat (YouTube-live style)**

A live comment stream inside an auction room, delivered over the existing auction WebSocket.

- `GET /api/v1/auctions/{id}/comments?limit=` — recent messages, oldest first (auction_management
  view).
- `POST /api/v1/auctions/{id}/comments` — post a message; it is pushed to everyone connected to the
  room (auction_management view - super admin now, bidders/sellers later).

How it works: comments are stored in `auction_comments` and broadcast on the same Redis channel the
auction room already uses, as a new `{"type": "comment", "comment": ...}` event alongside the
existing snapshot/bid/updated/ended events. Any client on the room WebSocket receives them live.
Frontend: a super-admin auction room at `app/(admin)/auctions/[id]/page.tsx` (opened by clicking an
auction card) with a left-side live chat panel; `useAuctionRoom` runs one socket for both auction
state and chat. Bidder/seller rooms (with bidding) reuse the same endpoints later.

**Web push notifications (device notifications, like WhatsApp/Teams web)**

Browser push that shows on the user's device even when the tab is in the background, wired to chat
and auction events and extensible to any module.

- `GET /api/v1/push/vapid-key` — the public VAPID key the browser needs to subscribe.
- `POST /api/v1/push/subscribe` — store a device subscription (body is the browser's
  `PushSubscription.toJSON()`: `{ endpoint, keys: { p256dh, auth } }`).
- `POST /api/v1/push/unsubscribe` — remove a device subscription by `endpoint`.

How it works:

- Which events push is controlled by one array, `PUSH_KINDS` in `app/services/notifications.py`.
  Any notification whose kind is in that array also fires a device push - so adding push to a new
  module is a one-line change (add its `NotificationKind` to the array). Currently enabled: outbid,
  auction won, auction lost, and new chat message.
- `notifications.push(...)` still writes the in-app notification (the bell) on the caller's
  transaction; the device push is sent in the background, best-effort, and never blocks or fails the
  request. Dead subscriptions (HTTP 404/410 from the push service) are pruned automatically.
- Sending uses the Web Push protocol with VAPID (pywebpush). If `PROVENIX_VAPID_PRIVATE_KEY` is
  unset, device push is silently disabled and only the in-app notification is written.
- Chat: sending a message now also raises a `new_message` notification to the recipient, so chat
  shows in the bell and as a device push.

Setup (one time): generate a VAPID key pair and set `PROVENIX_VAPID_PUBLIC_KEY`,
`PROVENIX_VAPID_PRIVATE_KEY` and (optionally) `PROVENIX_VAPID_SUBJECT`. Frontend glue lives in
`frontend/public/sw.js` (the service worker) and `frontend/lib/push.ts` (`enablePush` /
`disablePush`); call `enablePush(accessToken)` from a UI toggle after login.
...
- Repaired `main`: the "merge conflicts" commit had committed unresolved conflict markers into 9
  files (broke backend import + frontend build). Resolved every hunk keeping the teammate's block
  feature; my merged-in work was preserved in the unconflicted regions.
- Added an auction-room live comment stream (YouTube-live style). Backend: `auction_comments` table,
  `GET`/`POST /auctions/{id}/comments`, broadcast as a `comment` event on the existing room
  WebSocket. Frontend: a super-admin-only auction room page opened by clicking an auction card, with
  a left-side live chat panel and the auction's image, stats and bid activity. Migration 0012.
- The same endpoints and socket will serve bidder/seller rooms later (with bidding added).

### 2026-07-27 — web push notifications (device notifications)

Added browser push so users get device notifications (WhatsApp/Teams style) for chat and auction
events, extensible to any module via a single array.

- `push_subscriptions` table stores each device's subscription; `/push` endpoints handle the VAPID
  key, subscribe and unsubscribe. Sending uses the Web Push protocol with VAPID (pywebpush).
- `PUSH_KINDS` in `services/notifications.py` is the array that decides which notification kinds also
  fire a device push. Existing auction notifications flow through unchanged; a new `new_message`
  kind wires chat into both the bell and push. Adding a module = add its kind to the array.
- The device push is background and best-effort - it never blocks or fails the originating request,
  and dead endpoints are pruned. Disabled automatically when no VAPID key is configured.
- Minimal frontend glue added as new files only: `public/sw.js` and `lib/push.ts`. Migration 0011.

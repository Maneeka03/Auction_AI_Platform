...
from app.rbac.permissions import Role
from app.schemas.auction import AuctionOut, UpdateAuctionRequest
from app.schemas.bank_account import BankAccountOut, UpsertBankAccountRequest
from app.schemas.bid import ParticipantOut
from app.schemas.portal import (
    AuctionAnalysisOut,
    ReservePriceUpdate,
...
from app.schemas.property import AddPropertyImageRequest, CreatePropertyRequest, PropertyOut, UpdatePropertyRequest
from app.services import auctions, bank_accounts, bids, escrow, portal, properties
...
@router.get("/bank-details", response_model=BankAccountOut | None)
async def my_bank_details(
    session: DbSession, actor: CurrentUser, response: Response
) -> BankAccountOut | None:
    account = await bank_accounts.mine(session, actor.id)
    if account is None:
...
@router.put("/bank-details", response_model=BankAccountOut)
async def upsert_bank_details(
    payload: UpsertBankAccountRequest, session: DbSession, actor: CurrentUser
) -> BankAccountOut:
    if Role.SELLER not in actor.roles:
        raise AppError(
            status.HTTP_403_FORBIDDEN, "forbidden", "Only sellers can add payout details."
        )
    account = await bank_accounts.upsert(
        session,
        actor.id,

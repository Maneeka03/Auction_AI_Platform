...
from app.models.lead import LeadStatus
from app.models.user import User
from app.rbac.permissions import Access, Module, can_view_unanonymized_crm
from app.schemas.crm import BuyerCrmOut, BuyerCrmPage, SellerCrmOut, SellerCrmPage
from app.schemas.lead import CreateLeadRequest, LeadOut, LeadPage, UpdateLeadRequest
from app.services import crm, leads
...
async def list_buyers(
    ...
    rows, total = await crm.buyers(session, page, size, search)
    unanonymized = can_view_unanonymized_crm(actor.roles)
    return BuyerCrmPage(
        items=[BuyerCrmOut.of(*row, unanonymized=unanonymized) for row in rows],
        total=total,
        page=page,
        size=size,
    )
...
async def list_sellers(
    ...
    unanonymized = can_view_unanonymized_crm(actor.roles)
    return SellerCrmPage(
        items=[SellerCrmOut.of(*row, unanonymized=unanonymized) for row in rows],
        total=total,
        page=page,
        size=size,
    )
